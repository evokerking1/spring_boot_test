...
class TestcontainersConfiguration {
	...
	fun azuriteContainer(): GenericContainer<*> {
		return GenericContainer(DockerImageName.parse("mcr.microsoft.com/azure-storage/azurite:latest")).withExposedPorts(10000, 10001, 10002)
	}
	...
	@Bean
	@ServiceConnection
	fun milvusContainer(): MilvusContainer {
		return MilvusContainer(DockerImageName.parse("milvusdb/milvus:latest"))
	}
	...
	fun qdrantContainer(): QdrantContainer {
		return QdrantContainer(DockerImageName.parse("qdrant/qdrant:latest"))
	}
	...
	@Bean
	@ServiceConnection
	fun weaviateContainer(): WeaviateContainer {
		return WeaviateContainer(DockerImageName.parse("semitechnologies/weaviate:latest"))
	}
